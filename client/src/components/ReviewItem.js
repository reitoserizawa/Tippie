...
import EditRestaurantReviewForm from './EditRestaurantReviewForm'
import './RestaurantReviewItem.css'

function ReviewItem ({id, name, image, review, star, user_id, restaurant_id, worker_id, user, errors, setErrors}) {
        ...
        if (!user.id || user_id === user.id) {
            ...
            <div className="helpful-thumb text-body-2 ml-3">
                <span className="fit-icon thumbs-icon">
                    <button type="button" onClick={() => setShowEdit(!showEdit)} className="btn btn-info btn-sm"><i className="fa-solid fa-pen"></i></button>
                    <EditRestaurantReviewForm open={showEdit} onClose={() => setShowEdit(false)} id={id} user={user} star={star} formerReview={review} errors={errors} setErrors={setErrors} restaurant_id={restaurant_id} worker_id={worker_id}/>
                </span>
            </div> 
            <div className="helpful-thumb text-body-2 ml-3">
                <span className="fit-icon thumbs-icon">
                    <button onClick={deleteReview} type="button" className="btn btn-danger btn-sm"><i className="fa-solid fa-trash"></i></button>
                </span>
            </div>
            </>
        } else {
                ...
                <li>
                        <div className="right">
                            ...
                            <span className="gig-rating text-body-2">
                                <img src={image} className="profile-pict-img img-fluid" alt={name} style={{width: "32px", height: "32px", display: "block", borderRadius: "50%x"}} />
                            </span>
                                {name}
                                <span className="gig-rating text-body-2">
                            ...
                            <div className="review-description" style={{marginLeft:"50px", marginRight: "50px"}}>
                                <p>
                                    {review}
                                </p>
                            </div>
